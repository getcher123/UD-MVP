...
- Документы (PDF/DOC/PPT/PPTX/TXT/JPG/PNG) приводятся к промежуточный формат (PDF/Markdown) и проходят через AgentQL или ChatGPT в зависимости от настроек;
- TXT читается напрямую и передаётся в структурированный GPT-запрос без конвертации и AgentQL;
- JPG/PNG автоматически конвертируются в PDF и идут по vision-потоку (`pdf_to_images` → `vision_per_page` → `chatgpt_structured`);
...
- Для `xls` файлов с готовой структурой (`listing.xlsx`, `listings.xlsx`) вызывается CRM-пайплайн: пакет отправляется на `APP_CRM_URL`, в `data/results/<request_id>/` сохраняются `crm_request.json` и `crm_response.json`.
...
- Для `txt` файл просто читается и передаётся в `chatgpt_structured` без промежуточного AgentQL.
- Для `pdf` доступны стадии `pdf_to_images` (рендер страниц в PNG через Poppler) и `vision_per_page` (GPT-vision по промпту и схеме), после чего результат передаётся в `pdf.chatgpt_structured`.
- `image` (JPG/PNG) принудительно конвертируются в PDF и затем проходят тот же vision-поток (`pdf_to_images`, `vision_per_page`) и `pdf.chatgpt_structured`.
...
- Тесты лежат в `app-ms/tests` (юнит + интеграционные сценарии).
